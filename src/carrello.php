...
<div class="container">
    ...
    if ($res && $res->num_rows > 0) {
        echo "<table><tr><th>Prodotto</th><th>Quantità</th><th>Prezzo</th><th>Totale</th><th>Azione</th></tr>";
        while ($row = $res->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td><a href='?rimuovi={$row['idProdotto']}' class='btn-delete'>Rimuovi</a></td>
                  </tr>";
        }
        echo "<tr><td><strong>Totale Complessivo</strong></td><td><strong>€" . number_format($totale, 2) . "</strong></td></tr></table>";
        echo '<form method="POST"><button type="submit" name="acquista">Conferma Acquisto</button></form>';
    } else {
    ...
    if (isset($_POST['acquista']) && $totale > 0) {
        ...
        echo "<div class='scontrino'><h3>Scontrino</h3><hr>";
        while ($r = $resItems->fetch_assoc()) {
            $conn->query("INSERT INTO Dettaglio_acquisto (idAcquisto, idProdotto, quantita) VALUES ($idAcquisto, {$r['idProdotto']}, {$r['quantita']})");
            $conn->query("UPDATE Prodotti SET giacenza = giacenza - {$r['quantita']} WHERE idProdotto = {$r['idProdotto']}");
            echo "<p>{$r['nome']} - {$r['quantita']} x €" . number_format($r['prezzo'], 2) . " = €" . number_format($r['quantita'] * $r['prezzo'], 2) . "</p>";
        }
        $conn->query("DELETE FROM Carrello WHERE idCliente = $cliente");
        echo "<hr><h4>TOTALE: €" . number_format($totale, 2) . "</h4><p class='success'>Acquisto completato!</p></div>";
    }
    ?>
    <br><a href="acquisti.php" class="btn-link">⬅ Continua acquisti</a><br>
